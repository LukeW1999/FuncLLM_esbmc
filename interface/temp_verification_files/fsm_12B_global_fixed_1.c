...
void fsm_12B_global_step(void)
{
  ...
  /* State validation */
  if (rtDW.UnitDelay_DSTATE < 0.0 || rtDW.UnitDelay_DSTATE > 3.0) {
  ...
  /* If: '<S4>/If' */
  if (rtb_UnitDelay == STATE_TRANSITION) {
  ...
  } else if (rtb_UnitDelay == STATE_NOMINAL) {
  ...
  } else if (rtb_UnitDelay == STATE_MANEUVER) {
  ...
  } else if (rtb_UnitDelay == STATE_STANDBY) {
   ...
   *  Merge: '<S5>/Merge'
   */
  if (rtDW.Merge == STATE_TRANSITION) {
  ...
  } else if (rtDW.Merge == STATE_NOMINAL) {
  ...
  } else if (rtDW.Merge == STATE_MANEUVER) {
  ...
  } else if (rtDW.Merge == STATE_STANDBY) {
  ...
  if (rtDW.Merge == STATE_STANDBY && rtDW.Merge_p[1]) {
      rtDW.Merge_p[1] = false;
  }

  /* If: '<S14>/If' */
  if (rtb_UnitDelay == SENSTATE_NOMINAL) {
    ...
    if (rtU.limits) {
       ...
       */
      rtDW.Merge_g = SENSTATE_FAULT;
    } else if (!rtDW.Merge_p[1]) {
      /* Switch: '<S17>/Switch1' incorporates:
       *  Constant: '<S17>/Constant7'
       *  Merge: '<S14>/Merge'
       */
      rtDW.Merge_g = SENSTATE_TRANSITION;
    } else {
      /* Merge: '<S14>/Merge' incorporates:
       *  Switch: '<S17>/Switch1'
       */
  ...
  } else if (rtb_UnitDelay == SENSTATE_TRANSITION) {
  ...
  } else if (rtb_UnitDelay == SENSTATE_FAULT) {
   ...
   */
  rtDW.UnitDelay2_DSTATE = !(rtDW.Merge_g == SENSTATE_FAULT);
...
{
    rtDW.UnitDelay_DSTATE = STATE_TRANSITION;
    rtDW.UnitDelay1_DSTATE = SENSTATE_NOMINAL;
